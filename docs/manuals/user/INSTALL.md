...
1. **Limited Browser Testing**
    - As we start to build the frontend we have not yet tested styling and UI consistancy around different browsers.
2. **Refreshing Page**
    - Due to current issues with deployment, refreshing the page often invokes a 404 error.
3. **Final Challenge Execution**
    - When completing the final solution, it is possible to invoke an infinite for or while loop that slows down your browser.
